...
- 渲染属性(render props)，A组件接受一个函数参数，给这个函数一个值，render的时候返回这个函数，实际使用的时候这个函数参数可以返回B组件，并把函数参数给B组件props，相当于在要用的B组件外面套一层A组件负责注入props。缺点: 同上 `<Mouse render={mouse => (<Cat mouse={mouse/>)}/>`
- react hook
